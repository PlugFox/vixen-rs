...
impl CaptchaService {
    ...
    async fn generate(&self) -> Captcha {
        ...
        let digits_count: u8 = length as u8;
        let dist = Uniform::from(0..10);
        let numbers: Vec<u8> = (0..digits_count).map(|_| rng.sample(dist)).collect();
        let text = numbers.iter().map(|d| d.to_string()).collect::<String>();
        ...
        let encoder = Encoder::from_rgba(img.as_raw(), width, height);
        let webp = encoder.encode_lossy(75.0);
        let bytes = webp.to_vec();
